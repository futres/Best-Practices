...
data <- read.csv("https://de.cyverse.org/dl/d/39E97F88-8761-47B0-AAB0-99342711C3B0/data.for.analyses.csv", header = TRUE, stringsAsFactors = FALSE)
length(unique(data$scientificName)) #139

##combine with pantheria----
sp.data <- unique(data$scientificName) 
nrow(pan[pan$MSW05_Binomial %in% sp.data,]) #128 spp
data.mass <- data[data$measurementType == "mass",]

pan.data <- merge(pan, data.mass, by.x = "MSW05_Binomial", by.y = "scientificName", all.y = TRUE, all.x = FALSE)
length(unique(pan.data$MSW05_Binomial)) #139
...
pan.data.mass.sample <- pan.data %>%
  ...
  dplyr::summarise(sample.size.mass.pan = length(measurementValue[!is.na(mass)])) %>%
  as.data.frame()
